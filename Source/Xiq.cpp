...
bool Xiq::Update(GameTime time)
{
    ...
    Vector2 center = location + steer*sqrt(2.0f);
    Point2 target = center + Vector2(cos(angle), sin(angle));
    ...
    float dt = 1.0f/60.0f;
    Vector2 accel = force*(1.0f/mass);
    Point2 next_location = location*2.0f - last_pos + accel*dt*dt;

    Vector2 vel = (next_location - location).Normalised()*speed*time.DeltaSeconds();
