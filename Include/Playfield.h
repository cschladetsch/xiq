...
public:
    ...
    bool OutOfBounds(Point2 p) const;

    /// retrieve the element at a given location in the playfield
    Element At(Point2 const &P) const { return At(P.x, P.y); }
    Element At(int x, int y) const
    {
        return OutOfBounds(Point2(x,y)) ? None : elements.at(x + y*width);
    }
    ...
    Point2 *LineDraw(Point2 p0, Point2 p1, Point2 *out) const;
...
//protected:
    ...
    void Collision(Point2 p0, Element new_start, Element current);

    /// test two flood fills; return 0 if they are the same area, else 1 if seed1 is greater area than seed0
    bool FloodFillCompare(Point2 seed0, Point2 seed1);

    /// perform a flood fill
    int FloodFill(Point2 seed, Element type) { return FloodFill(seed.x, seed.y, type); }
    int FloodFill(int x, int y, Element type);

    /// set elements in the playfield
    void Set(Point2 P, Element E) { Set(P.x, P.y, E); }
    void Set(int x, int y, Element E) { if (OutOfBounds(Point2(x,y))) return; elements.at(x + y*width) = E; }
    ...
    int CalcNewArea(Direction dir, Point2 P);
